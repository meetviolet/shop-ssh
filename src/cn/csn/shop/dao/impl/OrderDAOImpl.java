...
import cn.csn.shop.dao.OrderDAO;
...
public class OrderDAOImpl implements OrderDAO {
	...
	public List<Order> findByPageUid(Integer uid, int begin, int pagesize) {
		...
		List<Order> list = (List<Order>) this.hibernateTemplate.execute(
				(HibernateCallback<Order>) new PageHibernateCallback(hql, new Object[] { uid }, begin, pagesize));
		return list;
	}
	...
	public List<Order> findWithPage(int begin, int pagesize) {
		...
		List<Order> list = (List<Order>) this.hibernateTemplate
				.execute((HibernateCallback<Order>) new PageHibernateCallback(hql, null, begin, pagesize));
		if (list != null && list.size() > 0) {
			return list;
		}
		return null;
