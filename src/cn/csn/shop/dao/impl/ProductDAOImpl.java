...
import cn.csn.shop.dao.ProductDAO;
...
public class ProductDAOImpl implements ProductDAO {
	...
	public List<Product> findHot() {
		...
		List<Product> list = (List<Product>) this.hibernateTemplate.findByCriteria(criteria, 0, 8);
		return list;
	}
	...
	public List<Product> findByPageCid(Integer cid, int begin, int pagesize) {
		...
		List<Product> list = (List<Product>) this.hibernateTemplate.execute(
				(HibernateCallback<Product>) new PageHibernateCallback(hql, new Object[] { cid }, begin, pagesize));
		return list;
	}
	...
	public List<Product> findByPageCsid(Integer csid, int begin, int pagesize) {
		...
		List<Product> list = (List<Product>) this.hibernateTemplate.execute(
				(HibernateCallback<Product>) new PageHibernateCallback(hql, new Object[] { csid }, begin, pagesize));
		return list;
	}
	
	@Override
	public int findByPname(String pname) {
	...
	@Override
	public List<Product> findByPagePname(String pname, int begin, int pagesize) {
		String hql = "select p from Product p where p.pname like '%" + pname + "%'";
		List<Product> list = (List<Product>) this.hibernateTemplate.execute(
				(HibernateCallback<Product>) new PageHibernateCallback(hql, new Object[] {}, begin, pagesize));
		return list;
	}
	...
	public List<Product> findWithPage(int begin, int pagesize) {
		...
		List<Product> list = (List<Product>) this.hibernateTemplate
				.execute((HibernateCallback<Product>) new PageHibernateCallback(hql, null, begin, pagesize));
		if (list != null && list.size() > 0) {
			return list;
		}
		return null;
	...
	public void update(Product product) {
		product = (Product) this.hibernateTemplate.getSessionFactory().getCurrentSession().merge(product);
